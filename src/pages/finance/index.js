...
const calculatorData = [
  ...
  },
  {
    ...
    image: 'https://th.bing.com/th/id/OIP.FoAnej_CGYSAzCqeCuAD0AHaFy?pid=ImgDet&rs=1',
  },
  {
    name: 'NPV Calculator - Net Present Value',
    calculatorId: "3",
    description: 'NPV formula',
    image: 'https://th.bing.com/th/id/R.d028ae9b3648e003cc4a57db88c92423?rik=4audq8XTnURT2Q&riu=http%3a%2f%2flh6.ggpht.com%2fmHH9JmPEqYMdpPtL-iZVejVsD7vleaJgdL2dspCjRJQrJ4xq57PeRnm59PV1xUvnRPn9%3dw300&ehk=%2fB60NaiCKs2yVfLjGDNBbQMdJeEl7475WsYCljAJSSw%3d&risl=&pid=ImgRaw&r=0',
  },
];
    ...
    <Layout>
      <div className="h-screen w-screen py-6 bg-yellow-50 flex items-center justify-center flex-wrap">
        {calculatorData.map(calculator => (
          <div className="bg-white w-72 h-96 shadow-md rounded m-3" key={calculator.name}>
            <div className="h-3/4 w-full">
              <img className="w-full h-full object-cover rounded-t" src={calculator.image} alt={calculator.name} />
            </div>
            <div className="w-full h-1/4 p-3">
              <Link href={`/finance/dynamicCalculatorPage?cal=${calculator.calculatorId}`}>
                <span className=" hover:text-yellow-600 text-gray-700">
                  <span className="text-lg font-semibold uppercase tracking-wide">{calculator.name}</span>
                </span>
              </Link>
              <p className="text-gray-600 text-sm leading-5 mt-1">{calculator.description}</p>
            </div>
            <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-2" onClick={() => handleGenerateEmbedCode(calculator.calculatorId)}>Create Link</button>
          </div>
        ))}
        <div>
